...
| `ieee_rounding`       | `bool`    | Controls whether the floating-point division follows the rounding behavior specified by the IEEE 754 standard                                                        |
...
Conclusion: Compared with GPU, Ascend lacks fp64 support but additionally supports fp16 and bf16.
...
The following example performs the element-wise fast division of the input tensors `x` and `y`:
