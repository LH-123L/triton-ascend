...
Description: Used to compare the elements of two tensors element-wise, equivalent to `>`.
...
| `builder` | - | Reserved parameter, external invocation not supported for now |
...
Conclusion: Compared with GPU, Ascend lacks support for uint16/uint32/uint64 and fp64.
...
Compared with GPU, Ascend lacks support for uint16/uint32/uint64 and fp64.

### 2.4 Usage

The following example performs a greater-than operation on the three-dimensional tensors `x0` and `x1`:
...
> Capability missing relative to the community but can be developed

Support for the uint8 type can be considered.
